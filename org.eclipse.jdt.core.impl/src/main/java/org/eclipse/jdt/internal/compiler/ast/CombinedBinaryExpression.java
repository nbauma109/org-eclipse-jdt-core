...
public void generateOptimizedStringConcatenation(BlockScope blockScope,
	...
	if (this.referencesTable == null || (this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT != OperatorIds.PLUS || (this.bits & ASTNode.ReturnTypeIDMASK) != TypeIds.T_JavaLangString) {
...
public void generateOptimizedStringConcatenationCreation(BlockScope blockScope,
	...
	if (this.referencesTable == null || (this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT != OperatorIds.PLUS || (this.bits & ASTNode.ReturnTypeIDMASK) != TypeIds.T_JavaLangString || this.constant != Constant.NotAConstant) {
  	...
  	for (restart = this.arity - 1; restart >= 0; restart--) {
 		if (((cursor = this.referencesTable[restart]).bits &
    	ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT != OperatorIds.PLUS || (cursor.bits & ASTNode.ReturnTypeIDMASK) != TypeIds.T_JavaLangString) {
