...
public class CompletionElementNotifier extends SourceElementNotifier {
	...
	protected char[][] getInterfaceNames(TypeDeclaration typeDeclaration) {
		...
		if (superInterfaces != null) {
			...
			next: for (int i = 0; i < superInterfacesLength; i++) {
				...
				if (superInterface instanceof CompletionOnKeyword || CompletionUnitStructureRequestor.hasEmptyName(superInterface, this.assistNode)) continue next;
	...
	protected char[] getSuperclassName(TypeDeclaration typeDeclaration) {
		...
		if (superclass instanceof CompletionOnKeyword || CompletionUnitStructureRequestor.hasEmptyName(superclass, this.assistNode)) return null;
	...
	protected char[][] getThrownExceptions(AbstractMethodDeclaration methodDeclaration) {
		...
		if (thrownExceptions != null) {
			...
			next : for (int i = 0; i < thrownExceptionLength; i++) {
				...
				if (thrownException instanceof CompletionOnKeyword || CompletionUnitStructureRequestor.hasEmptyName(thrownException, this.assistNode)) continue next;
	...
	protected char[][] getTypeParameterBounds(TypeParameter typeParameter) {
		...
		if (firstBound != null) {
			...
			} else if (!CompletionUnitStructureRequestor.hasEmptyName(firstBound, this.assistNode)) {
	...
	protected void notifySourceElementRequestor(AbstractMethodDeclaration methodDeclaration, TypeDeclaration declaringType, ImportReference currentPackage) {
		if (methodDeclaration instanceof CompletionOnMethodReturnType || methodDeclaration instanceof CompletionOnMethodTypeParameter || methodDeclaration instanceof CompletionOnMethodName) return;
	...
	protected void notifySourceElementRequestor(FieldDeclaration fieldDeclaration, TypeDeclaration declaringType) {
		if (fieldDeclaration instanceof CompletionOnFieldType || fieldDeclaration instanceof CompletionOnFieldName) return;
	...
	protected void notifySourceElementRequestor(ImportReference importReference, boolean isPackage) {
		if (importReference instanceof CompletionOnKeyword2 || (importReference instanceof CompletionOnImportReference ||
        		importReference instanceof CompletionOnPackageReference) && importReference.tokens[importReference.tokens.length - 1].length == 0) return;
