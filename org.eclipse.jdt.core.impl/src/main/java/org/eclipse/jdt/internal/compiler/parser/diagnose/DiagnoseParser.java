...
//
	private RepairCandidate primaryPhase(int error_token) {
		...
		// the error token.
		//
		if (repair.code == INSERTION_CODE && this.buffer[repair.bufferPosition - 1] == 0) {
...
//
	private RepairCandidate secondaryPhase(int error_token) {
		...
		// then it is chosen.
		//
		if (misplaced.distance > MIN_DISTANCE && (misplaced.numDeletions <= repair.numDeletions ||
	...
	private int parseCheck(int[] stck, int stack_top, int first_token, int buffer_position) {
		...
		process_terminal: for (;;) {
			...
			do {
				...
				if(act <= NUM_RULES && Parser.rules_compliance[act] > this.options.sourceLevel) {
                 	return 0;
                }
				int lhs_symbol = Parser.lhs[act];
				act = this.tempStackTop > max_pos
	...
	private void reportPrimaryError(int msgCode, int nameIndex, int token, int scopeNameIndex) {
		...
		switch(msgCode) {
			...
			case SCOPE_CODE:
		                ...
		                if (tmpAddedToken > -1) {
		                	...
		                	if(template != null) {
			                	for (int element : template) {
		                			...
		                			addedTokens[addedTokenCount++] = element;
								}
		                	} else {
			                	addedTokenCount = 0;
			                	addedTokens = null;
				...
				} else if(this.reportProblem) problemReporter().parseErrorInsertToCompleteScope(
                	errorStart,
                	errorEnd,
                	buf.toString());
			...
			case SCOPE_CODE:
	            ...
	            if (scopeNameIndex != 0) {
	                ...
	                if(this.reportProblem) problemReporter().parseErrorInsertToComplete(
						...
						buf.toString(),
						Parser.readableName[scopeNameIndex]);
	            } else if(this.reportProblem) problemReporter().parseErrorInsertToCompletePhrase(
                	errorStart,
                	errorEnd,
                	buf.toString());
	            break;
