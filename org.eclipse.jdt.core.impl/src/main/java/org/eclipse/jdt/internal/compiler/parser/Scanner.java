		...
		if (previous != '@') {
			nextTag : for (int itag = 0; itag < this.taskTags.length; itag++) {
				...
				for (int t = 0; t < tagLength; t++) {
                    ...
                    if ((sc = src[i + t]) != (tc = tag[t]) && (this.isTaskCaseSensitive || ScannerHelper.toLowerCase(sc) != ScannerHelper.toLowerCase(tc))) {
                    	continue nextTag;
                    }
				}
...
public final int getLineEnd(int lineNumber) {
	...
	if (this.lineEnds == null || this.linePtr == -1 || lineNumber > this.lineEnds.length+1 || lineNumber <= 0)
		return -1;
	...
	while (true) { //loop for jumping over comments
		...
		do {
			...
			if (checkIfUnicode) {
                ...
                isWhiteSpace = switch (this.currentCharacter) {
...
/*					if (this.sourceLevel >= ClassFileConstants.JDK1_5) {
				...
				default :
					...
					if (c < ScannerHelper.MAX_OBVIOUS) {
                        ...
                        if ((ScannerHelper.OBVIOUS_IDENT_CHAR_NATURES[c] & ScannerHelper.C_DIGIT) != 0) {
			...
			NextToken: switch (this.currentCharacter) {
				...
				case '"' :
					...
					try {
                        ...
                        if (isTextBlock && firstClosingBrace > 0) {
	...
	switch (this.currentCharacter) {
		...
		default :
            ...
            if (ScannerHelper.isDigit(this.currentCharacter = this.source[this.currentPosition++])) {
            	...
            	if (digit >= 0 && digit <= 7) {
            		number = number * 8 + digit;
            		if (!ScannerHelper.isDigit(this.currentCharacter = this.source[this.currentPosition++]) || zeroToThreeNot) {// has read \NotZeroToThree OctalDigit Digit --> ignore last character
                    	this.currentPosition--;
                    } else {
	...
	if (!dotPrefix && this.currentCharacter == '0') {
		if (getNextChar('x', 'X') >= 0) { //----------hexa-----------------
            ...
            if (getNextChar('.')) {
				...
				if (hasNoDigitsBeforeDot && end == start || getNextChar('p', 'P') < 0) {
					if (this.sourceLevel < ClassFileConstants.JDK1_5) {
...
private static class Goal {
	...
	static {
		...
		for (int i = 1; i <= ParserBasicInformation.NUM_RULES; i++) {  // 0 == $acc
			...
			if ("TypePattern".equals(Parser.name[Parser.non_terminal_index[Parser.lhs[i]]]) || "PrimaryPattern".equals(Parser.name[Parser.non_terminal_index[Parser.lhs[i]]]) || "GuardedPattern".equals(Parser.name[Parser.non_terminal_index[Parser.lhs[i]]]) || "Pattern".equals(Parser.name[Parser.non_terminal_index[Parser.lhs[i]]])) //$NON-NLS-1$
				patternStates.add(i);
            else if ("ParenthesizedPattern".equals(Parser.name[Parser.non_terminal_index[Parser.lhs[i]]])) //$NON-NLS-1$
				patternStates.add(i);
		}
...
int disambiguatedToken(int token, Scanner scanner) {
	...
	if (token == TokenNameARROW  &&  mayBeAtCaseLabelExpr() &&  scanner.caseStartPosition < scanner.startPosition) {
		...
		token = TokenNameAT308;
		if (maybeAtEllipsisAnnotationsStart() && parser.parse(Goal.VarargTypeAnnotationGoal) == VanguardParser.SUCCESS) {
