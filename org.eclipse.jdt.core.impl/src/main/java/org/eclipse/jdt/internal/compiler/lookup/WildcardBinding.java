...
//						}
	...
	public char[] computeUniqueKey(boolean isLeaf) {
        ...
        wildCardKey = switch (this.boundKind) {
            case Wildcard.UNBOUND -> TypeConstants.WILDCARD_STAR;
            case Wildcard.EXTENDS -> CharOperation.concat(TypeConstants.WILDCARD_PLUS, this.bound.computeUniqueKey(false/*not a leaf*/)); /*not a leaf*/
            default -> CharOperation.concat(TypeConstants.WILDCARD_MINUS, this.bound.computeUniqueKey(false/*not a leaf*/)); /*not a leaf*/
        };
	...
	public boolean isProperType(boolean admitCapture18) {
		...
		this.inRecursiveFunction = true;
		try {
			if (this.bound != null && !this.bound.isProperType(admitCapture18) || this.superclass != null && !this.superclass.isProperType(admitCapture18))
				return false;
			if (this.superInterfaces != null)
                for (ReferenceBinding element : this.superInterfaces)
	...
	public boolean mentionsAny(TypeBinding[] parameters, int idx) {
		...
		this.inRecursiveFunction = true;
		try {
			if (super.mentionsAny(parameters, idx) || this.bound != null && 	this.bound.mentionsAny(parameters, -1))
				return true;
			if (this.otherBounds != null) {
				for (TypeBinding otherBound : this.otherBounds)
