...
public class ClassFile implements TypeConstants, TypeIds {
	...
	public void addAttributes() {
		...
		if ((this.produceAttributes & ClassFileConstants.ATTR_SOURCE) != 0) {
			...
			if (lastIndex != -1) {
				fullFileName = fullFileName.substring(lastIndex + 1);
			}
		...
		if (numberOfInnerClasses != 0) {
			...
			Arrays.sort(innerClasses, (o1, o2) -> {
	...
	private RecordComponent getRecordComponent(ReferenceBinding declaringClass, char[] name) {
		if (declaringClass instanceof SourceTypeBinding) {
			...
			if (rcb != null) {
				return rcb.sourceRecordComponent();
			}
		}
		return null;
	}
...
//						stackMapFrameCodeStream.addExceptionMarker(handlerPC, exceptionLabel.exceptionType);
	...
	public int generateTypeAnnotationsOnCodeAttribute() {
		...
		int size = allTypeAnnotationContexts.size();
		return completeRuntimeTypeAnnotations(attributesNumber,
															null,
															node -> size > 0,
															() -> allTypeAnnotationContexts);
	}
...
//						stackMapFrameCodeStream.addExceptionMarker(handlerPC, exceptionLabel.exceptionType);
				...
				if (exceptionLabel != null) {
					...
					while  (iRange < maxRange) {
						...
						if (exceptionLabel.exceptionType == null) {
							...
							int nameIndex = switch (exceptionLabel.exceptionType.id) {
                                case T_null -> this.constantPool.literalIndexForType(ConstantPool.JavaLangClassNotFoundExceptionConstantPoolName);
                                case T_long -> this.constantPool.literalIndexForType(ConstantPool.JavaLangNoSuchFieldErrorConstantPoolName);
                                default -> this.constantPool.literalIndexForType(exceptionLabel.exceptionType);
                            };
							this.contents[localContentsOffset++] = (byte) (nameIndex >> 8);
							this.contents[localContentsOffset++] = (byte) nameIndex;
						}
	...
	public void completeMethodInfo(
		...
		if ((this.produceAttributes & ClassFileConstants.ATTR_TYPE_ANNOTATION) != 0) {
			...
			if (methodDeclaration != null) {
				...
				if ((lambda.bits & ASTNode.HasTypeAnnotations) != 0 && lambda.arguments != null)
	...
	private void generateElementValue(int attributeOffset, Expression defaultValue, Constant constant, TypeBinding binding) {
		...
		switch (binding.id) {
			...
			case T_JavaLangString :
				this.contents[this.contentsOffset++] = (byte) 's';
				int stringValueIndex =
					this.constantPool.literalIndex(constant.stringValue().toCharArray());
				if (stringValueIndex == -1) {
					if (!this.creatingProblemType) {
						// report an error and abort: will lead to a problem type classfile creation
						TypeDeclaration typeDeclaration = this.referenceBinding.scope.referenceContext;
	...
	private int generateLocalVariableTableAttribute(int code_length, boolean methodDeclarationIsStatic, boolean isSynthetic) {
		...
		for (int i = 0, max = this.codeStream.allLocalsCounter; i < max; i++) {
			LocalVariableBinding localVariable = this.codeStream.locals[i];
			int initializationCount = localVariable.initializationCount;
			if (initializationCount == 0 || localVariable.declaration == null) continue;
	...
	public int generateMethodInfoAttributes(MethodBinding methodBinding) {
		...
		if (this.targetJDK >= ClassFileConstants.JDK1_4) {
			...
			if (methodDeclaration != null) {
				...
				if (lambda != null && (methodBinding.tagBits & TagBits.HasParameterAnnotations) != 0) {
	...
	private void generateMissingTypesAttribute() {
		...
		if (initialSize > 1) {
			Collections.sort(this.missingTypes, (o1, o2) -> CharOperation.compareTo(o1.constantPoolName(), o2.constantPoolName()));
		}
		int previousIndex = 0;
	...
	private boolean jdk16packageInfoAnnotation(final long annotationMask, final long targetMask) {
		return this.targetJDK <= ClassFileConstants.JDK1_6 &&
	...
	private int generateRuntimeAnnotations(final Annotation[] annotations, final long targetMask) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (annotationMask != 0 && (annotationMask & targetMask) == 0 && !jdk16packageInfoAnnotation(annotationMask, targetMask)) continue;
		...
		if (needSynthetics) {
			...
			if (syntheticArgumentTypes != null) {
				for (int i = 0, count = syntheticArgumentTypes.length; i < count; i++) {
					...
					boolean couldForwardToMandated = !anonymousWithNestedSuper || declaringClass.superclass().enclosingType().equals(syntheticArgumentTypes[i]);
	...
	private List<StackMapFrame> filterFakeFrames(Set<Integer> realJumpTargets, Map<Integer, StackMapFrame> frames, int codeLength) {
		...
		Collections.sort(result, (frame, frame2) -> frame.pc - frame2.pc);
		return result;
	}
	...
	private StackMapFrame createNewFrame(int currentPC, StackMapFrame frame, boolean isClinit, MethodBinding methodBinding) {
		...
		// initialize locals
		initializeLocals(isClinit || methodBinding.isStatic(), currentPC, newFrame);
		return newFrame;
	}
